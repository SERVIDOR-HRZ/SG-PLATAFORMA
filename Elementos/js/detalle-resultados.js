    ...
    const html = `
        <div class="modal-tab-content active">
            ...
            <div class="preguntas-container">
                ${data.preguntas.map((pregunta, index) => renderizarPreguntaCard(pregunta, index + 1)).join('')}
            </div>
        </div>
    `;
...
function renderizarTabPreguntas(datos) {
    ...
    const html = `
        <div class="modal-tab-content active">
            <div class="preguntas-container">
                ${todasLasPreguntas.map((pregunta, index) => renderizarPreguntaCard(pregunta, index + 1)).join('')}
            </div>
        </div>
    `;
        ...
        return `
                        <div class="${clases.join(' ')}" style="${esRespuestaEstudiante ? 'background: rgba(255, 165, 0, 0.4) !important; border: 4px solid #ffa500 !important;' : ''}">
                            ...
                            <div class="opcion-indicador ${esRespuestaCorrecta ? 'correcta' : 'incorrecta'}">
                                ...
                                <span class="opcion-porcentaje" title="${cantidadReal} estudiante(s) seleccionaron esta opción">${porcentajeReal}%</span>
                            </div>
                        </div>
                    `;
    }).join('')}
